...
from my_todolists.entities_db import TodoListDB, TaskDB
from my_todolists.logic import OldListsManager, TxtSaver
from my_todolists.user_inputs import UserTaskInput
...
class TodoList:
    ...
    def __init__(self) -> None:
        ...
        self.all_lists = OldListsManager().list_folder_content()

    def create_list(self):
        """Create a new To-Do list."""
        new_list = TodoListDB(self.id_list + 1, dict())
        TxtSaver().save_list(new_list.id_list, new_list.tasks)
        return new_list.id_list

    def add_task_to_list(
        self, title: str, description: str, tags: list
    ) -> int:
        ...
        try:
            ...
            old_tasks = 0
        new_task = TaskDB(
            old_tasks + 1, title, description, tags
        )
        tasks[new_task.id_task] = [
            new_task.title, new_task.description, new_task.tags
        ]
        TxtSaver().save_tasks(self.id_list, tasks)
    ...
    def search_list_id(self, id_list_choice: str) -> None:
        ...
        id_list_match = [
            ...
            ).list_folder_content() if file == id_list_choice
        ]
        if id_list_match == []:
            return False
    ...
    def show_all_lists(self) -> None:
        ...
        for file in OldListsManager().list_folder_content():
            self.id_list = file
            for task, title, description, tags in self.current_tasks():
...
class Task:
    ...
    def modify_task(
        ...
        TxtSaver().save_tasks(list_choice, tasks)
        return (
