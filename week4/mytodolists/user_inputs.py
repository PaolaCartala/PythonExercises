...
class UserMainInput:
    ...
    def choice_task_title():
        task_title = CustomValidators().validator_whitespaces(
    ...
    def choice_task_description():
        task_desc = CustomValidators().validator_whitespaces(
...
class UserListInput:
    ...
    def new_list_params():
        """User inputs title"""
        list_title = CustomValidators().validator_whitespaces(
...
class UserQueriesInput:

    @staticmethod
    def input_tag_group():
        tag = CustomValidators().validator_whitespaces(
            input("\nInsert the tag you want to group by:\n")
        )
        while not tag:
            tag = CustomValidators().validator_whitespaces(
                input("\nInsert the tag you want to group by:\n").capitalize()
